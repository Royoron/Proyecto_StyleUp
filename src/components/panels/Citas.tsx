...
export function PanelMisCitas() {
  const { citas, cancelarCita, navegarA } = useApp();
    ...
    <div className="tarjeta-servicio" style={{ height: 'auto' }}>
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <Button variante="nav" onClick={() => navegarA('reservar')}>
          <i className="bi bi-plus me-1" />Nueva cita
        </Button>
      </div>
      ...
      <table className="tabla-citas">
        ...
        <tbody>
          {citas.length === 0 ? (
            <tr>
              <td colSpan={6} style={{ color: 'var(--gris)', textAlign: 'center', padding: '1.5rem' }}>
    ...
    <div className="tarjeta-servicio" style={{ height: 'auto' }}>
      ...
      <table className="tabla-citas">
        ...
        <tbody>
          {historial.map(h => (
            <tr key={h.id_cita}>
              <td>{getNombreEspecialidad(h.id_especialidad)}</td>
              <td>{getNombreBarbero(h.cedula_barbero)}</td>
