...
import {
  ...
  deleteHorario,
  getHorarioById,
  listHorariosByBarbero,
  replaceHorarioSemanalBarbero,
  updateHorario,
} from "./horario.service.js";

function assertPuedeGestionarHorario(req: Request, cedula_barbero: string) {
  if (req.user?.role === "SUPERADMIN") return;
  if (req.user?.role === "BARBERO" && req.user.sub === cedula_barbero) return;
...
export async function listHorariosByBarberoController(req: Request, res: Response, next: NextFunction) {
  try {
...
export async function replaceHorarioBarberoController(req: Request, res: Response, next: NextFunction) {
  try {
    const cedula_barbero = String(req.params.cedula_barbero);
    assertPuedeGestionarHorario(req, cedula_barbero);
    const horarios = await replaceHorarioSemanalBarbero(
      cedula_barbero,
      req.body.horario,
    );
    return res.json(horarios);
