...
export async function createCitaTx(input) {
    const fecha = input.fecha instanceof Date
        ? input.fecha
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (barbero.id_especialidad !== input.id_especialidad) {
            throw new ApiError(StatusCodes.BAD_REQUEST, "El barbero seleccionado no pertenece a la especialidad elegida");
        }
        ...
        if (citaExistente) {
            throw new ApiError(StatusCodes.CONFLICT, "El barbero ya tiene una cita en esa fecha y hora");
        }
...
export async function cancelarCita(id_cita) {
...
export async function actualizarEstadoCita(id_cita, estado) {
    const cita = await prisma.cita.findUnique({ where: { id_cita } });
    if (!cita)
        throw new ApiError(StatusCodes.NOT_FOUND, "Cita no encontrada");
    return prisma.cita.update({ where: { id_cita }, data: { estado } });
}
export async function listarCitasPorDia(fechaIso) {
...
export async function listarCitasPorBarbero(cedula_barbero) {
    return prisma.cita.findMany({
